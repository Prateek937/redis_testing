...
module.exports.reshard = (nodeTo, portTo, nodeFrom, portFrom, next) => {
    ...
    waterfall([
        ...
        (clusterToId, next) = run(clusterFromIdCommand, float(clusterToId, (err, result) => err ? next(err) : next(null, result.trim()))),
        (clusterToId, clusterFromId, next) => {
            const finalCommand = `redis-cli --cluster reshard ${nodeTo}:${portTo} --cluster-from ${clusterFromId} --cluster-to ${clusterToId} --cluster-slots 4096 --cluster-yes | grep Ready`;
